...
class InjuriesRepo {
  ...
  Future<Either<Failure, List<QuestionModel>>> getInjuryQuestions() async {
    try {
      ...
      if ((result.statusCode ?? 400) < 400) {
        final questions = (result.data['data'] as List)
  ...
  Future<Either<Failure, void>> updateAffectedBodyParts(
      List<String> parts) async {
    try {
  ...
  Future<Either<Failure, List<BodyParts>>> getAffectedBodyParts() async {
    try {
      ...
      if ((result.statusCode ?? 400) < 400) {
        ...
        return Right(
            (data).map((e) => BodyParts.fromValue(e.toString())).toList());
      } else {
        return Left(ServerFailure(result.data.toString()));
      }
    } catch (e) {
