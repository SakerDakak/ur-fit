...
import '../data/data_source/meals_local_datasource.dart';
...
class MealsRepo {
  ...
  Future<Either<Failure, List<MealModel>>> searchRecipes(
      {required SearchRecipeModel searchRecipeModel}) async {
    try {
      final result = await mealsRemoteDataSource.searchRecipes(
  ...
  Future<Either<Failure, MealPlanModel>> generateMealPlan(
      {required double targetCalories}) async {
    try {
      final result = await mealsRemoteDataSource.generateMealPlan(
  ...
  Future<Either<Failure, void>> addLocalNutritionData(
      NutritionData data) async {
    try {
      await mealsLocalDatasource.addData(data);
