...
class AuthRepo {
  ...
  Future<Either<Failure, void>> sendCode({required String email}) async {
    try {
      await _apiClient
          .post(EndPoints.sendEmailActivationCode, data: {'email': email});
      return const Right(null);
    } on Failure catch (e) {
      return Left(e);
    }
  ...
  Future<Either<Failure, UserModel>> otpCheckCode(
      {required String email, required String code}) async {
    try {
      final result = await _apiClient.post(EndPoints.otpCheckCode,
          data: {'email': email, 'otpCode': code});
  ...
  Future<Either<Failure, UserModel>> otpForgetPassword(
      {required String email, required String code}) async {
    try {
      final result = await _apiClient.post(EndPoints.otpForgetPassword,
          data: {'email': email, 'otpCode': code});
      if (result.data['status'] == "unverified code") {
        throw BadRequestException(L10n.tr().unverifiedCode);
      } else {
  ...
  Future<Either<Failure, ({UserModel user, String token})>> register(
      RegisterRequest registerModel) async {
    try {
      final response = await _apiClient.post(EndPoints.register,
          data: await registerModel.toJson());
  ...
  Future<Either<Failure, String>> resendOtp({required String email}) async {
    try {
      final response =
          await _apiClient.post(EndPoints.resendOTP, data: {'email': email});
  ...
  Future<Either<Failure, List<CountryModel>>> getCities(
    ...
    try {
      final response = await _apiClient
          .get(EndPoints.cities, parameter: {'country_id': countryId});
  ...
  Future<Either<Failure, ({UserModel user, String token, String? message})>>
      _sendSocialAuth(
  ...
  Future<Either<Failure, ({UserModel user, String token, String? message})>>
    ...
    try {
      ...
      if (googleAuth == null) {
        return left(ServerFailure(L10n.tr().googleAuthenticationFailed));
      }
      final req = RegisterRequest(
          name: googleUser?.displayName, email: googleUser?.email);
      return await _sendSocialAuth(
          accessToken: googleAuth.accessToken!, req: req);
